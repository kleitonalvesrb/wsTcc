...
public class WSCliente {
	...
	/**
	 * Método responsavel por recuperar a senha do usuário, irá verificar se existe usuario
	 * com o email informado, caso exista irá trazer rodos os dados do usuário solicitado
	 * enviará a senha para o email juntamente com a data da solicitação
	 * @param email
	 * @return O retorno será apenas para validação na parte de quem solicita, caso nao exista
	 * dados com o email informado o usuário sera retornado como vazio, caso exista será retornado
	 * usuário com o email preenchido
	 */
	@GET
	...
	public Usuario recuperaSenhaUsuario(@PathParam("email") String email){
		...
		// Se encontrar usuario com o email solicitado buscara a senha e enviará o email 
		if (uDao.verificaExistencia(email)){
			Usuario usuarioRecuperado = new Usuario(); // Usuario recuperado no banco
			usuarioRecuperado = uDao.buscaUsuarioEmail(email);// usuario recuperado do banco preenchido
			...
			u.setEmail(email); // preenche o usuario apenas com o email e retornar para o app para confirmar que o email solicitado existe
		}else{
			u.setEmail("");
		}
		return u;
	 ...
	 * @param email
	 * @return Retornará o usuario vazio caso não seja encontrado ou usuario preenchido apenas com o email caso seja encontrado
	 */
	@GET
